rankhospital <- function(state,outcome, num = "best") {
      ...
      hospital.rank <- outcome.data[order(outcome.data[,3],outcome.data[,1]),]
      if (num == "best"){
            hospital.ranked.name <- hospital.rank[1,1]
      } else if (num == "worst") {
            hospital.ranked.name <- hospital.rank[nrow(hospital.rank),1]
      } else if (num > nrow(hospital.rank)) {
            hospital.ranked.name <- NA
      } else {
            hospital.ranked.name <- hospital.rank[num,1]
      }
      as.character(hospital.ranked.name)
}